const ui = (() => {
  ...
  var sendFor = "server"; // "server" or "spreadsheet"

  const sendScore = (data) => {
    console.log(scorePostIp, playerId);
    console.log(now_map, pre_map);

    if (!playerId || (!spreadSheet && !scorePostIp)) return;

    // sendForの決定を三項演算子で簡潔に
    sendFor = spreadSheet ? "spreadsheet" : "server";
    console.log("sendFor", sendFor);

    // データのセット
    Object.assign(data.status.performance, {
      keyScore: keyScoreValue,
      keyArr: keyArr,
    ...
    const postScore = async () => {
      try {
        const response = await fetch(scorePostIp, {
          method: "POST",
        ...
        if (sendFor === "server") {
          ...
          if (result.status !== "success") {
            ...
            document.getElementById("percentage").style.color = "#5AFF19";
    ...
    // spreadsheetの場合はawait不要
    if (sendFor === "spreadsheet") {
      fetch(scorePostIp, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
  ...
  const performance = (() => {
    ...
    if (html_id["raw_score"])
      var raw_score = document.getElementById("raw_score");
    ...
    if (html_id["energy_bar"])
      var energy_bar = document.getElementById("energy_bar");
    if (html_id["mod_nf"]) var mod_nf = document.getElementById("mod_nf");
    if (html_id["key_score"])
      var key_score = document.getElementById("key_score");
    ...
    return (data) => {
      ...
      if (html_id["raw_score"])
        raw_score.innerText = format(performance.rawScore);
      ...
      if (typeof performance.softFailed !== "undefined") {
        if (performance.softFailed === true) {
          ...
          if (html_id["energy_group"] && energy_display)
            energy_group.setAttribute("style", "visibility: hidden");
          if (html_id["mod_nf"]) {
      ...
      if (html_id["percentage"]) {
        ...
        percentage.innerText =
          (performance.currentMaxScore > 0 ? score_num.toFixed(2) : "100.00") +
          "%";
      }
      if (html_id["key_score"]) {
      ...
      if (now_energy !== null) {
        ...
        if (html_id["energy_bar"])
          energy_bar.setAttribute("style", `width: ${Math.round(now_energy)}%`);
      }

      if (typeof op_performance !== "undefined")
  ...
  const timer = (() => {
    ...
    function update(time) {
      ...
      if (html_id["progress"])
        bar.setAttribute(
          "style",
          `stroke-dashoffset: ${(1 - percentage) * circumference}px`
        );
      if (typeof op_timer_update !== "undefined")
      ...
      if (progress != display) {
        ...
        if (typeof op_timer_update_sec !== "undefined")
    ...
    return {
      start(time, length, speed) {
        ...
        if (html_id["song_length"])
          song_length.innerText = `${length_min}:${length_sec}`;
  ...
  const beatmap = (() => {
    ...
    if (html_id["mapper_header"])
      var mapper_header = document.getElementById("mapper_header");
    if (html_id["mapper"]) var mapper = document.getElementById("mapper");
    if (html_id["mapper_footer"])
      var mapper_footer = document.getElementById("mapper_footer");

    if (html_id["difficulty"])
    ...
    if (html_id["pre_bsr_text"])
      var pre_bsr_text = document.getElementById("pre_bsr_text");
    if (html_id["energy"]) var energy = document.getElementById("energy");
    if (html_id["energy_group"])
      var energy_group = document.getElementById("energy_group");
    ...
    return (data) => {
      ...
      console.log(diff_time)
      timer.start(
        beatmap.start + diff_time,
        beatmap.length,
        mod_data.songSpeedMultiplier
      );
      ...
      if (html_id["energy_group"])
        energy_group.setAttribute("style", `visibility: ${visibility}`);
      if (beatmap.difficulty === "ExpertPlus") {
        beatmap.difficulty = "Expert+";
      }

      if (html_id["image"])
        cover.setAttribute("src", `data:image/png;base64,${beatmap.songCover}`);
      ...
      httpRequest.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          if (typeof op_beatsaver_res !== "undefined")
      ...
      if (pre_songHash === beatmap.songHash) {
        ...
        if (
          bsr_display &&
          beatmap.songHash !== null &&
          beatmap.songHash.match(/^[0-9A-F]{40}/i)
        ) {
          httpRequest.open(
            "GET",
            beatsaver_url + beatmap.songHash.substr(0, 40),
            true
          );
      ...
      if (beatmap.levelAuthorName) {
        if (html_id["mapper_header"])
          mapper_header.innerText = mapper_header_org;
        if (html_id["mapper"]) mapper.innerText = beatmap.levelAuthorName;
        if (html_id["mapper_footer"])
          mapper_footer.innerText = mapper_footer_org;
      } else {
  ...
  return {
    ...
    show() {
      document.getElementById("percentage").color = "white";
